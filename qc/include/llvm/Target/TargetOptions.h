  ...
  public:
    TargetOptions()
        : PrintMachineCode(false), NoFramePointerElim(false),
          ...
          UseInitArray(false), MergeFunctions(false),
          ShrinkWrapCommonArgs(false), LongCall(false), TrapFuncName(""),
    ...
    /// MergeFunctions - Merge similar functions.
    unsigned MergeFunctions : 1;

    /// ShrinkWrapCommonArgs - If a function is called several times with
    /// the same compile-time constant values for the same arguments,
    /// generate wrappers that only take the variable arguments and call
    /// the original function with the full set. Replace the original calls
    /// with the calls to the wrappers. This saves code size.
    unsigned ShrinkWrapCommonArgs : 1;

    /// LongCall - This flag enable generation of long calls.
    unsigned LongCall : 1;
