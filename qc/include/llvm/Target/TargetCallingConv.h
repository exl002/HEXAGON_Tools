...
namespace ISD {
  ...
  private:
    ...
    static const uint64_t InAgg          = 3ULL << 12;
    static const uint64_t InAggOffs      = 12;
  ...
  public:
    ...
    enum AggregateElementPosition {
      BeginningOfAggregate = 1,
      MiddleOfAggregate = 2,
      EndOfAggregate = 3
    };

    bool isInAggregate() const { return Flags & InAgg; }
    bool isBeginningOfAggregate() const {
      return ((Flags & InAgg) >> InAggOffs) == BeginningOfAggregate;
    }
    bool isMiddleOfAggregate() const {
      return ((Flags & InAgg) >> InAggOffs) == MiddleOfAggregate;
    }
    bool isEndOfAggregate() const {
      return ((Flags & InAgg) >> InAggOffs) == EndOfAggregate;
    }
    void setInAggregate(AggregateElementPosition Pos) {
      Flags &= ~(0x3ULL << InAggOffs);
      Flags |= ((uint64_t)Pos) << InAggOffs;
    }
    void setBeginningOfAggregate() { setInAggregate(BeginningOfAggregate); }
    void setMiddleOfAggregate() { setInAggregate(MiddleOfAggregate); }
    void setEndOfAggregate() { setInAggregate(EndOfAggregate); }
