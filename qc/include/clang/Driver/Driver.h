...
private:
  ...
  /// Certain options suppress the 'argument unused during compilation' warning.
  bool SuppressMissingOptionsWarning : 1;
...
private:
  ...
  bool CheckInitialPhase(types::ID InputType, const Arg *InputArg,
                         phases::ID InitialPhase, phases::ID FinalPhase,
                         const Arg *FinalPhaseArg, const ArgList &Args) const;

  phases::ID BuildPipeline(
    const llvm::SmallVector<phases::ID, phases::MaxNumberOfPhases> &PL,    
    phases::ID FinalPhase,
    const ArgList &Args,
    OwningPtr<Action> *Current) const;
...
public:
  ...
  /// PrintVersion - Print the driver version, target, and thread model.
