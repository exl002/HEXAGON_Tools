...
public:
  ...
  /// isLegalPostIncrementLoad - Return true if an indexed load with
  /// increment are legal for this target.
  virtual bool isLegalPostIncrementLoad(Type *Ty) const {
    return false;
  }

  /// isLegalPostIncrementStore - Return true if an indexed store with
  /// increment is legal for this target.
  virtual bool isLegalPostIncrementStore(Type *Ty) const {
    return false;
  }
  ...
  /// Inline threshold.
  virtual unsigned getInlineThreshold(unsigned OptLevel,
                                      unsigned OptSize) const {
    unsigned Threshold = 225;
    if (OptSize == 1) // -Os
      Threshold = 75;
    else if (OptSize == 2) // -Oz
      Threshold = 25;
    else if (OptLevel >= CodeGenOpt::Aggressive) // -O3
      Threshold = 275;
    return Threshold;
  }

  /// Return true if the target can perform unrolling of loops with
  /// run-time trip counts.  This function allows the target to
  /// set the unroll factor (Count) and threshold values too.
  virtual bool canUnrollRuntime(Loop *L, Pass *P,
                                unsigned &Count, unsigned &Threshold) const {
    return false;
  }
