...
int _Tlsset(_Tlskey_t, const void *);
...
typedef pthread_key_t _Tlskey_t;
  ...
  #define _Tlsget(key) pthread_getspecific(key)
   ...
   #if  _GLOBAL_LOCALE
     typedef void *_Tlskey_t;

     #define _Tlsalloc(key, dtor)
     #define _Tlsfree(key)
     #define _Tlsset(key, value)
     #define _Tlsget(key)
   #else
     typedef void *_Tlskey_t;

     extern int sys_Tlsalloc(_Tlskey_t *, _Tlsdtor_t);
     extern int sys_Tlsfree(_Tlskey_t);
     extern int sys_Tlsset(_Tlskey_t, const void *);
     extern void *sys_Tlsget(_Tlskey_t);

     #define _Tlsalloc(key, dtor)     sys_Tlsalloc(key, dtor)
     #define _Tlsfree(key)            sys_Tlsfree(key)
     #define _Tlsset(key, value)      sys_Tlsset(key, value)
     #define _Tlsget(key)             sys_Tlsget(key)
   #endif
