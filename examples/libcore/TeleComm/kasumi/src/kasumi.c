...
static UWord16 KLi1[8], KLi2[8];
static UWord16 KOi1[8], KOi2[8], KOi3[8];
static UWord16 KIi1[8], KIi2[8], KIi3[8];
 ...
 *---------------------------------------------------------------------*/
static UWord16 FI( UWord16 in, UWord16 subkey )
{
	UWord16 nine, seven;
	static UWord16 S7[] = {
	...
	nine  = (UWord16)(in>>7);
	seven = (UWord16)(in&0x7F);

	/* Now run the various operations */

	nine  = (UWord16)(S9[nine]  ^ seven);
	seven = (UWord16)(S7[seven] ^ (nine & 0x7F));
 ...
 *---------------------------------------------------------------------*/
static UWord32 FO( UWord32 in, int index )
{
	UWord16 left, right;
 ...
 *		appropriate subkeys to use.
 *---------------------------------------------------------------------*/
static UWord32 FL( UWord32 in, int index )
{
	UWord16 l, r, a, b;
	...
	a  = (UWord16) (l & KLi1[index]);
	r ^= ROL16(a,1);

	b  = (UWord16)(r | KLi2[index]);
	l ^= ROL16(b,1);
...
void KeySchedule( UWord8 *k )
{
	...
	k16 = (WORD *)k;
	for( n=0; n<8; ++n )
		key[n] = (UWord16)((k16[n].b8[0]<<8) + (k16[n].b8[1]));
