...
public:
  ...
  /// replaceVRegUsesWith - Replace all instances of FromReg with ToReg in the
  /// machine function. This expects SSA form, handles only virtual registers
  /// and unlike replaceRegWith, it does not change any definitions of FromReg.
  void replaceVRegUsesWith(unsigned FromReg, unsigned ToReg);
  ...
  /// getVRegDef - Return the machine instr that defines the specified subreg
  /// of the specified virtual register. It returns null if no such instruction
  /// is found. This assumes that the code is in SSA form, so there should only
  /// be one definition.
  MachineInstr *getVRegDef(unsigned Reg, unsigned SubReg) const;
  ...
  void dumpDefs(unsigned RegNo) const;
#endif
