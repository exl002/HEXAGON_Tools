...
public:
  ...
  /// This function returns true if target allows memset to be lowered by
  /// vectorized stores. Otherwise it will return false.
  /// @brief Determine if the target can generate vector stores for memset.
  virtual bool
    canMemsetLowerToVectorStores(SDValue Src,
                                 uint64_t NumberOfBytesToWrite) const {
    return false;
  }
  ...
  /// Return true if the target wants aggressive post-increment transformations.
  virtual bool allowAggressivePostInc() const {
    return false;
  }

  /// shouldEmitAlignment - Return true if the target wants to output an
  /// alignment directive for the function.
  virtual bool shouldEmitAlignment(const Function &) const {
    return true;
  }
...
public:
  ...
  /// Thresholds for inlining.
  virtual unsigned getInlineThreshold(unsigned OptLevel, unsigned OptSize) const {
    unsigned Threshold = 225;
    if (OptSize == 1) // -Os
      Threshold = 75;
    else if (OptSize == 2) // -Oz
      Threshold = 25;
    else if (OptLevel > 2) // -O3
      Threshold = 275;
    return Threshold;
  }
