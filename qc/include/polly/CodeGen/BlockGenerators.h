...
typedef std::vector<Value*> ValueVectorT;
typedef DenseMap<const Value*, ValueVectorT> MapValueVectorT;
...
///
/// The only public function exposed is generate().
class VectorBlockGenerator : protected BlockGenerator {
public:
  ...
  /// @param P          A reference to the pass this function is called from.
  ///                   The pass is needed to update other analysis.
  /// @Reductions       A map that associates the location of a reduction
  ///                   (scalar) to the location of the corresponding vector.

  static void generate(IRBuilder<> &B, ScopStmt &Stmt,
                       VectorValueMapT &GlobalMaps,
                       __isl_keep isl_map *Schedule,
                       Pass *P, MapValueVectorT &Reductions) {
...
protected:
  ...
  ///  Base variable points to the starting position from which Polly builds
  ///  the vectors.
  int Base;

  //ElementsPerVector indicates how many elements can fit in the vector.
  int ElementsPerVector;

  /// Reduction maps the location of the reduction variable to its
  /// corresponding vector location.
  /// Reduction:  Alloca i32 --> Alloca <4 x i32>
  ...
  void copyCmpInst(const CmpInst *Inst, ValueMapT &VectorMap,
                   VectorValueMapT &ScalarMaps, bool IsFloatCmp);

  void copySelectInst(const SelectInst *Inst, ValueMapT &VectorMap,
                      VectorValueMapT &ScalarMaps);

  void copyInstScalarized(const Instruction *Inst, ValueMapT &VectorMap,
                          VectorValueMapT &ScalarMaps, bool HasVectorOperand);
