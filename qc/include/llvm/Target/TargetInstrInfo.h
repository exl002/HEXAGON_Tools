...
public:
  ...
  /// and check if the loop is finished.  Return the register that contains
  /// the result of the check.  We need this function when peeling off one
  /// or more iterations of a loop.
  ...
  /// shouldSplitMBBAfterCalls - Return true if  basic blocks should be split
  /// after function calls (for alignment).
  virtual bool shouldSplitMBBAfterCalls(MachineFunction &MF) const {
    return true;
  }
...
public:
  ...
  virtual bool canExecuteInBundle(const MachineInstr *First,
  ...
  /// findInstrPredicate - Set the the predicate of the instruction
  /// in the specified predicate vector
  virtual void findInstrPredicate(MachineInstr *MI,
                                 SmallVectorImpl<MachineOperand> &Cond) const
  { }

  /// isCondPrerequisiteForPred - Return true if the first predicate
  /// is required for the second predicate to be true (i.e., the
  /// second predicate is a boolean AND of the first predicate)
  virtual bool isCondPrerequisiteForPred(SmallVectorImpl<MachineOperand> &Pred1,
                                         SmallVectorImpl<MachineOperand> &Pred2) const
  {
    return true;
  }
  ...
  /// regHasSizeImpact - return the number of extra bytes if any operand of MI
  /// is assigned a physical register that has a costPerUse.
  virtual unsigned regHasSizeImpact(MachineInstr *MI) const {
    return 0;
  }
  ...
  /// isReadOnly - Return true when MI is a load from read only memory.
  virtual bool isReadOnly(const MachineInstr *MI) const {
    return false;
  }
  ...
  virtual bool getBaseAndOffsetPosition(MachineInstr *MI, unsigned &BasePos,
