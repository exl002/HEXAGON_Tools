...
public:
  ...
  /// Return true if the basic block is a loop header block that is in a loop
  /// that has been unrolled.  We use this flag to ensure that a loop is
  /// unrolled only once.
  bool isUnrolled() const { return SubclassOptionalData != 0 ; }

  /// Mark the basic block as a loop header block that has been unrolled.
  /// We use this flag to ensure that a loop is unrolled only once.
  void setUnrolled() { SubclassOptionalData = 1; }
